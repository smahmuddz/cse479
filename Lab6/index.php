...
<?php
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
		...
		$brief = trim($brief);
		$brief = str_ireplace(array_keys($offensiveWords), array_values($offensiveWords), $brief);
		if (preg_match("/^\s*(?:\S+\s+){0,50}\S+\s*$/", $brief)) {
            $brief_er = "";
        } else {
	...
	<div>
        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
            ...
            echo "Name error: " . $name . "<br>";
            echo "Email error: " . $email . "<br>";
            echo "Password error: " . $pass . "<br>";
            echo "IP error: " . $ip . "<br>";
            echo "URL error: " . $url . "<br>";
            echo "Date of Birth error: " . $dob . "<br>";
            echo "Gender error: " . $gender . "<br>";
            echo "Mobile Number error: " . $mobile . "<br>";
            echo "Brief Info error: " . $brief. "<br>";
        }
        ?>
    </div>
    ...
    <div class="error">
        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
			...
			echo "Database summer2023 created successfully.";  
			}else{  
			echo "Sorry, database creation failed ".mysqli_error($conn);  
			}  
			
			//Create Table
			$sql = "create table user (name VARCHAR(20) NOT NULL,email VARCHAR(20),ip VARCHAR(20),web VARCHAR(20),age int,gender VARCHAR(10),mobile VARCHAR(20),bio VARCHAR(200));";  
			if(mysqli_query($conn, $sql)){  
